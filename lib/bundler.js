        ...
        shim(deps, shims, function(err) {
            ...
            (function next(deps, prev_offset) {
                deps.forEach(function(dep) {
                    ...
                    // only used for relative requires
                    if (prev_offset && id[0] === '.') {
                        id = prev_offset + id;
                    }

                    // if we have already seen a file
                    // we may still need an alias since our lookup
                    ...
                    if (seen[full_path || id]) {
                        ...
                        var seen_alias = aliases[full_path];
                        if (seen_alias !== id) {
                            // exported aliases
                            _aliases.push({
                                name: id,
                    ...
                    // true native dependency (not shimmed)
                    if (!dep.filename) {
                        val.name = dep.id;
                        val.offset = '';
                    }
