...
Bundle.prototype.generate = function(cb) {
    ...
    self.resolve(function(err, result) {
        if (err) {
            return cb(err);
        }
        ...
        if (dep.filename) {
            ...
            if (pkg_path) {
                ...
                for (var key in pkg_info.shims) {
                    shims[key] = path.join(path.dirname(pkg_path), pkg_info.shims[key])
                }
            }
        }
